    ...
    public static CompetitiveSwimmer matchIDFromResult(Result result) {
        ...
        for (Member m : memberList) {
            if (m instanceof CompetitiveSwimmer) {
                // This ensures that m is a Members.CompetitiveSwimmer or a subclass of it
                CompetitiveSwimmer swimmer = (CompetitiveSwimmer) m;  // Cast to Members.CompetitiveSwimmer
                if (result.getSwimmerID() == m.getMemberID()) {
                    return swimmer;  // Return the Members.CompetitiveSwimmer if IDs match
                }
            }
        }
